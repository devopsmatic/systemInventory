...
function Normalize-InstallDate {
    ...
    $s = [string]$d
    if ($s -match '^\d{8}$') {
        try { return ([datetime]::ParseExact($s,'yyyyMMdd',$null)).ToString('yyyy-MM-dd') } catch { return $s }
    }
...
function Show-Step {
    ...
    Write-Host ("STEP {0}/{1} - {2} - {3}% complete" -f $i,$t,$title,$pct) -ForegroundColor White
    Write-Progress -Activity "Collect-DevProfile" -Status $title -PercentComplete $pct -Id 1
}
...
$zipDestination = Join-Path "C:\" ("$computerName.zip")
...
try {
    ...
    if ($appx) {
        ...
        Write-Host "No Appx packages or Get-AppxPackage failed." -ForegroundColor Yellow
    }
} catch {
    Show-Warn ("Get-AppxPackage failed: " + $_.Exception.Message)
}
#endregion
    ...
    if ($wingetCmd) {
        ...
        if ($json) {
            try {
                ...
                Show-Warn ("winget parse failed: " + $_.Exception.Message)
            }
        } else {
            Write-Host "winget returned no JSON; skipping." -ForegroundColor Yellow
        }
    } else {
        Write-Host "winget not found; skipping." -ForegroundColor Yellow
    }
} catch {
    Show-Warn ("winget invocation failed: " + $_.Exception.Message)
}
#endregion
...
try {
    ...
    Show-Warn ("ProgramFiles scan failed: " + $_.Exception.Message)
}
#endregion
...
if ($FullMode) {
    ...
    try {
        ...
        if ($allPkgs) {
            ...
            Write-Host "Get-Package returned nothing." -ForegroundColor Yellow
        }
    } catch {
        Show-Warn ("Get-Package full mode failed: " + $_.Exception.Message)
    }
} else {
...
try {
    ...
    Show-Err ("Dedup failed: " + $_.Exception.Message)
}
#endregion
...
try {
    ...
    Show-Err ("CSV export failed: " + $_.Exception.Message)
}

# Try Excel for software (conditional install)
try {
    ...
    Show-Warn ("Software Excel export failed: " + $_.Exception.Message)
}
...
try {
    ...
    Show-Warn ("Env export failed: " + $_.Exception.Message)
}
#endregion

#region Misc exports: Git, Python, Node, VSCode
# Git
try {
...
# Python detection (parallel jobs, pip freeze per interpreter)
try {
    $pythonInterpreters = @()
    try { $pyOut = & py -0p 2>$null } catch { $pyOut = $null }
    if ($pyOut) {
        $lines = $pyOut -split "`n" | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne "" }
        foreach ($ln in $lines) { if ($ln -match '^\s*(?:-p)?\s*(.+)$') { $path = $Matches[1].Trim(); if (Test-Path $path) { $pythonInterpreters += (Resolve-Path $path).Path } } }
    }
...
try {
    ...
    if ($codeCmd) {
        ...
        if ($exts) { $exts | Set-Content -Path $vExts -Encoding UTF8 -Force } else { Set-Content -Path $vExts -Value "No extensions returned." -Encoding UTF8 -Force }
    } else {
        Set-Content -Path $vExts -Value "VSCode CLI not found." -Encoding UTF8 -Force
    }
...
$currentStep++; Show-Step -i $currentStep -t $currentStep -title "Writing summary and consolidated CSV (software)"
try {
    ...
    Show-Warn ("Summary write failed: " + $_.Exception.Message)
}
#endregion
...
try {
    ...
    try {
        $summaryRow | Export-Csv -Path $hwCsvSummary -NoTypeInformation -Encoding UTF8 -Force
        Write-Host "hwinfo.csv written" -ForegroundColor Green
    } catch {
        Log-HwError "Failed writing hwinfo.csv" $_
    }
...
#region Zip and finish
$currentStep++; Show-Step -i $currentStep -t $totalSteps -title "Zipping output folder"
try {
    # compress the top-level computer folder C:\<ComputerName>
    $sourceFolder = Join-Path "C:\" $computerName
    if (Test-Path $sourceFolder) {
        if (Test-Path $zipDestination) { Remove-Item -Path $zipDestination -Force -ErrorAction SilentlyContinue }
        Compress-Archive -Path (Join-Path $sourceFolder '*') -DestinationPath $zipDestination -Force -ErrorAction Stop
        Write-Host "Created ZIP: $zipDestination" -ForegroundColor Green
    } else {
        Write-Host "Source folder to zip not found: $sourceFolder" -ForegroundColor Yellow
    }
} catch {
    Show-Warn ("ZIP failed: " + $_.Exception.Message)
}
#endregion
